...
export function useAuth() {
  ...
  const login  = () => { window.location.href = `${API_BASE}/auth/spotify`; };
  const logout = useCallback(() => { localStorage.removeItem('djdou_token'); setUser(null); }, []);
...
export function useDNA() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    catch (err) { if (err.response?.status !== 404) setError(err.response?.data?.error || 'Failed'); }
    finally { setLoading(false); }
  }, []);
  ...
  const computeDNA = useCallback(async () => {
    ...
    catch (err) { setError(err.response?.data?.error || 'Failed to compute DNA'); throw err; }
...
export function useRecommendations() {
  ...
  const searchByMood = useCallback(async (query) => {
    setMoodLoading(true); setMoodResults(null);
    try { const { data } = await api.post('/recommendations/mood', { query }); setMoodResults(data); }
    catch (err) { setError(err.response?.data?.error || 'Mood search failed'); }
    finally { setMoodLoading(false); }
  }, []);

  return { recommendations, loading, error, fetchRecommendations, moodResults, moodLoading, searchByMood };
}
...
export function useSeeds() {
  ...
  const fetchSeeds = useCallback(async () => {
    setLoading(true);
    try { const { data } = await api.get('/seeds'); setSeeds(data); }
    finally { setLoading(false); }
  }, []);
  ...
  return {
    ...
    loading, adding, addSeed, removeSeed,
  };
}
...
export function usePlayer() {
  ...
  useEffect(() => {
    ...
    return () => { cancelAnimationFrame(rafRef.current); audio.pause(); audio.src = ''; };
  }, []);
  ...
  const play = useCallback((track) => {
    ...
    if (currentTrack?.spotifyId === track.spotifyId) {
      if (a.paused) { a.play(); setIsPlaying(true); rafRef.current = requestAnimationFrame(tick); }
      else { a.pause(); setIsPlaying(false); cancelAnimationFrame(rafRef.current); }
      return;
    }
    cancelAnimationFrame(rafRef.current);
    a.pause(); a.src = track.previewUrl; a.currentTime = 0;
    setCurrentTrack(track); setProgress(0);
    a.play().then(() => { setIsPlaying(true); rafRef.current = requestAnimationFrame(tick); }).catch(() => {});
  }, [currentTrack, tick]);

  const pause = useCallback(() => {
    audioRef.current?.pause(); setIsPlaying(false); cancelAnimationFrame(rafRef.current);
  }, []);
...
export function describeFeature(key, value) {
  const d = {
  ...
  const s = d[key];
  if (!s) return `${Math.round(value * 100)}%`;
  return s[Math.min(4, Math.floor(value * 5))];
}
