...
export function useAuth() {
  ...
  return {
    user, loading,
    login:   () => { window.location.href = `${API_BASE}/auth/spotify`; },
...
export function useSeeds() {
  ...
  const addSeed = useCallback(async (spotifyId) => {
    setAdding(true);
    try { const { data } = await api.post('/seeds', { spotifyId }); setSeeds((p) => [data, ...p]); }
...
export function useTrackSearch() {
  ...
  const clear = useCallback(() => {
    clearTimeout(timer.current); setQuery(''); setResults([]); setLoading(false);
  }, []);
