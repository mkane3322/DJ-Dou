...
import { useAuthCtx, usePlayerCtx, useTrackSearch, FEATURE_META } from '../lib';

export function Spinner({ size = 36, fullPage = false }) {
  const el = <div style={{ width:size, height:size, borderRadius:'50%', border:'3px solid var(--bg-overlay)', borderTop:'3px solid var(--accent)', animation:'spin 0.8s linear infinite', boxShadow:'0 0 12px rgba(168,85,247,0.3)' }} />;
...
export function EmptyState({ icon = '🎵', title, desc, action }) {
  return (
    <div style={{ textAlign:'center', padding:'4rem 1.5rem', display:'flex', flexDirection:'column', alignItems:'center', gap:'0.75rem' }}>
      ...
      {action && <button onClick={action.onClick} style={{ marginTop:'0.5rem', padding:'0.6rem 1.5rem', background:'linear-gradient(135deg,var(--accent),var(--accent-hot))', color:'#fff', borderRadius:'var(--radius-full)', fontFamily:'var(--font)', fontWeight:700, fontSize:'0.9rem', boxShadow:'0 0 20px rgba(168,85,247,0.3)' }}>{action.label}</button>}
    </div>
  );
}
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'5px' }}>
      ...
      <div style={{ height:'5px', background:'var(--bg-overlay)', borderRadius:'99px', overflow:'hidden' }}>
        <div style={{ height:'100%', width:`${value * 100}%`, background:`linear-gradient(90deg,${meta.color}88,${meta.color})`, boxShadow:`0 0 8px ${meta.color}55`, borderRadius:'99px', transition:'width 0.8s var(--ease)' }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ position:'relative' }}>
      <div style={{ display:'flex', alignItems:'center', gap:'8px', background:'var(--bg-base)', border:'1px solid var(--border-bright)', borderRadius:'var(--radius)', padding:'0 12px' }}>
        ...
        <input value={query} onChange={(e) => search(e.target.value)} onFocus={() => setFocused(true)} onBlur={() => setTimeout(() => setFocused(false), 200)} placeholder={placeholder} style={{ flex:1, border:'none', background:'none', outline:'none', color:'var(--text-primary)', fontFamily:'var(--font)', fontSize:'0.9rem', padding:'0.75rem 0' }} />
            ...
            return (
              <div key={track.spotifyId} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 14px', borderBottom:'1px solid var(--border)' }}>
                <div style={{ position:'relative', cursor:'pointer', flexShrink:0 }} onClick={() => track.previewUrl && play(track)}>
                  {track.albumArt ? <img src={track.albumArt} alt="" style={{ width:40, height:40, borderRadius:'var(--radius-sm)', objectFit:'cover' }} /> : <div style={{ width:40, height:40, borderRadius:'var(--radius-sm)', background:'var(--bg-overlay)', display:'flex', alignItems:'center', justifyContent:'center' }}>🎵</div>}
                  {track.previewUrl && <div style={{ position:'absolute', inset:0, borderRadius:'var(--radius-sm)', background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.8rem', color:'#fff' }}>{playing ? '⏸' : '▶'}</div>}
                </div>
                ...
                <button onClick={() => !seeded && onSelect && onSelect(track)} disabled={seeded} style={{ padding:'0.2rem 0.7rem', border:'1px solid', borderRadius:'var(--radius-full)', fontSize:'0.72rem', fontFamily:'var(--mono)', fontWeight:600, cursor: seeded ? 'default' : 'pointer', background: seeded ? 'rgba(163,230,53,0.15)' : 'rgba(168,85,247,0.15)', color: seeded ? 'var(--accent-lime)' : 'var(--accent)', borderColor: seeded ? 'rgba(163,230,53,0.4)' : 'var(--border-bright)' }}>
                  {seeded ? '✓ Added' : '+ Add'}
                </button>
              </div>
            );
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 1.5rem', height:60, display:'flex', alignItems:'center', gap:'2rem' }}>
        ...
        <div style={{ display:'flex', gap:'1.5rem', flex:1 }}>
          {[['Dashboard','/dashboard'],['Discover','/discover']].map(([label,to]) => (
            <NavLink key={to} to={to} style={({ isActive }) => ({ fontWeight:600, fontSize:'0.875rem', color: isActive ? 'var(--accent)' : 'var(--text-secondary)', transition:'color 150ms' })}>{label}</NavLink>
          ))}
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:'12px', marginLeft:'auto' }}>
          <div onClick={() => navigate('/profile')} style={{ width:32, height:32, borderRadius:'50%', background:'linear-gradient(135deg,var(--accent),var(--accent-hot))', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:'0.875rem', cursor:'pointer', overflow:'hidden' }}>
            {user?.avatar ? <img src={user.avatar} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : user?.displayName?.[0] || '?'}
          </div>
          <button onClick={logout} style={{ border:'1px solid var(--border)', color:'var(--text-muted)', fontFamily:'var(--mono)', fontSize:'0.7rem', padding:'0.3rem 0.75rem', borderRadius:'var(--radius-full)', cursor:'pointer' }}>Sign out</button>
        </div>
      </div>
    </nav>
  );
...
export function MiniPlayer() {
  ...
  return (
    <div style={{ position:'fixed', bottom:0, left:0, right:0, height:70, background:'rgba(8,7,15,0.95)', backdropFilter:'blur(20px)', borderTop:'1px solid var(--border)', display:'flex', alignItems:'center', gap:'1rem', padding:'0 1.5rem', zIndex:100 }}>
      ...
      <button onClick={() => isPlaying ? pause() : play(currentTrack)} style={{ width:40, height:40, borderRadius:'50%', background:'var(--accent)', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1rem', boxShadow:'0 0 16px rgba(168,85,247,0.4)', flexShrink:0 }}>{isPlaying ? '⏸' : '▶'}</button>
      <div style={{ flex:1, display:'flex', alignItems:'center', gap:'10px' }}>
        <div onClick={(e) => { const r = e.currentTarget.getBoundingClientRect(); seek((e.clientX - r.left) / r.width); }} style={{ flex:1, height:4, background:'var(--bg-overlay)', borderRadius:'2px', cursor:'pointer' }}>
      ...
      {isPlaying && (
        <div style={{ display:'flex', alignItems:'center', gap:'2px', height:20, flexShrink:0 }}>
          {[0,0.15,0.3].map((d) => <span key={d} style={{ width:3, height:16, background:'var(--accent)', borderRadius:'2px', display:'inline-block', transformOrigin:'center', animation:`waveBar 0.5s ${d}s ease-in-out infinite` }} />)}
        </div>
      )}
    </div>
  );
