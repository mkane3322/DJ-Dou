...
async function getSpotifyClient(user) {
  ...
  if (user.tokenExpiresAt && new Date() >= user.tokenExpiresAt) {
    const r = await spotifyTokenRequest({ grant_type: 'refresh_token', refresh_token: user.spotifyRefreshToken });
    token = r.access_token;
    user.spotifyAccessToken = token;
    user.tokenExpiresAt = new Date(Date.now() + r.expires_in * 1000);
    await user.save();
  }
  return axios.create({ baseURL: 'https://api.spotify.com/v1', headers: { Authorization: `Bearer ${token}` } });
}
...
async function getAudioFeatures(client, ids) {
  ...
  for (let i = 0; i < ids.length; i += 100) {
    const d = await spotifyGet(client, '/audio-features', { ids: ids.slice(i, i + 100).join(',') });
...
async function computeDNAVector(user, client) {
  const [short, medium, long] = await Promise.all([
    spotifyGet(client, '/me/top/tracks', { time_range: 'short_term',  limit: 50 }),
    spotifyGet(client, '/me/top/tracks', { time_range: 'medium_term', limit: 50 }),
    spotifyGet(client, '/me/top/tracks', { time_range: 'long_term',   limit: 50 }),
  ]);

  const map = new Map();
  for (const { items, w } of [{ items: short.items, w: 3 }, { items: medium.items, w: 2 }, { items: long.items, w: 1 }]) {
  ...
  const afs    = await getAudioFeatures(client, unique.map(({ t }) => t.id));
...
function interpretDNA(vec) {
  const [dance,energy,,,, speech, acoustic, instrumental, live, valence, tempo] = vec;
  return {
    danceability:     dance        > 0.6 ? 'highly danceable'    : dance > 0.4        ? 'moderately danceable' : 'not very danceable',
    energy:           energy       > 0.6 ? 'high energy'         : energy > 0.4       ? 'moderate energy'      : 'calm / low energy',
    valence:          valence      > 0.6 ? 'happy and positive'  : valence > 0.4      ? 'mixed tone'           : 'melancholic or dark',
    tempo:            tempo        > 0.6 ? 'fast-paced'          : tempo > 0.4        ? 'mid-tempo'            : 'slow',
    acousticness:     acoustic     > 0.6 ? 'very acoustic'       : acoustic > 0.4     ? 'somewhat acoustic'    : 'electronic/produced',
    instrumentalness: instrumental > 0.5 ? 'mostly instrumental' : 'vocal-forward',
    speechiness:      speech       > 0.5 ? 'rap/spoken word'     : 'sung vocals',
    liveness:         live         > 0.7 ? 'live recordings'     : 'studio recordings',
  };
}

async function findSimilarTracks(dnaVector, { limit = 20, excludeIds = [], featureFilters = {} } = {}) {
  ...
  return candidates
    .map((t) => ({ track: t, similarityScore: cosineSimilarity(dnaVector, t.featureVector) }))
...
async function generateDNASummary(interpretation, displayName) {
  const traits = Object.entries(interpretation).map(([k, v]) => `${k}: ${v}`).join('\n');
...
async function upsertTrack(spotifyTrack, audioFeatures) {
  ...
  return Track.findOneAndUpdate(
    { spotifyId: formatted.spotifyId },
    {
      ...formatted,
      audioFeatures: {
        danceability: audioFeatures.danceability, energy: audioFeatures.energy,
        key: audioFeatures.key, loudness: audioFeatures.loudness,
        mode: audioFeatures.mode, speechiness: audioFeatures.speechiness,
        acousticness: audioFeatures.acousticness, instrumentalness: audioFeatures.instrumentalness,
        liveness: audioFeatures.liveness, valence: audioFeatures.valence,
        tempo: audioFeatures.tempo, duration_ms: audioFeatures.duration_ms,
...
module.exports = {
  getAuthUrl, exchangeCode, getSpotifyClient, spotifyGet, getAudioFeatures, formatTrack,
  computeDNAVector, interpretDNA, findSimilarTracks, buildChartData,
  generateDNASummary, generateWhyText, parseMoodQuery,
  upsertTrack, cosineSimilarity,
};
